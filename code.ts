type CssOutputMode = "preserve" | "resolved";
...
function channelTo255(value: number): number {
...
function colorToCss(color: RGBA | RGB, format: "hex" | "rgb" = "hex"): string {
	...
	if (format === "rgb") {
		if (a < 1) return `rgba(${r}, ${g}, ${b}, ${alphaToString(a)})`;
		return `rgb(${r}, ${g}, ${b})`;
	}
...
function isColor(value: VariableValue): value is RGBA | RGB {
	return (
		typeof value === "object" &&
		value !== null &&
		"r" in value &&
...
function variableNameToCss(
	variable: Variable,
	collectionMap: Map<string, VariableCollection>,
): string {
	const collection = collectionMap.get(variable.variableCollectionId);
	const collectionName = slugify(collection?.name || "tokens");
...
function modeSelector(modeName: string, totalModes: number): string {
	const n = modeName.trim().toLowerCase();

	if (totalModes === 1) return ":root";

	if (n === "light" || n === "default" || n === "base") return ":root";
	if (n === "dark") return `[data-theme="dark"]`;

	return `[data-theme="${slugify(modeName)}"]`;
}

function variableValueToLiteral(value: VariableValue): string | null {
	if (typeof value === "boolean") return value ? "true" : "false";
	if (typeof value === "number") return String(value);
	if (typeof value === "string") return value;
	if (isColor(value)) return colorToCss(value, "hex");
	return null;
}
...
async function resolveToFinalLiteral(
	variable: Variable,
	modeId: string,
	...
	const effectiveModeId = await findFallbackModeId(
		variable,
		modeId,
		collectionMap,
	);
	if (!effectiveModeId) return null;
...
async function exportVariablesToCss(
	outputMode: CssOutputMode = "preserve",
): Promise<string> {
	...
	for (const variable of variables) {
		const list = varsByCollection.get(variable.variableCollectionId) ?? [];
		list.push(variable);
